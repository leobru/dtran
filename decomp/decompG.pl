...
$code = oct($1) if $prog =~ /Code start: (\d+)/;

print STDERR "Code start at $code decimal\n";
...
$prog =~ s@,AOX,C/0022;([^;]+);,YTA,64-40@\1@g;

# Handle multiplication in the (*=M+*) mode
$prog =~ s@,AOX,C/0022;(,NTR,3;)?([^;]+);14,VJM,P/0060@\2@g;
$prog =~ s@\(74000C\)@_NIL@g;
$prog =~ s@\(26624\)@_NIL@g;
...
if (%routines) {
    ...
    $prog =~ s@L($pattern)([^0-7])@$routines{$1}."(* $1 *)".$2@ge;
    map { 
...
$prog =~ s@12,VJM,P/1365 *@CALL ГГ@g;
$prog =~ s@12,VJM,P/0053 *@CALL ГГ@g;
$prog =~ s@14,VJM,P/1104 *@CALL MAPГA@g;
$prog =~ s@14,VJM,P/1115 *@CALL MAPЯГА@g;
...
$prog =~ s@14,VJM,P/0032 *@rewrite(output)@g;
...
$prog =~ s@14,VJM,P/0050 *@CALL mapia@g;
$prog =~ s@14,VJM,P/1363 *@CALL mapai@g;
$prog =~ s@14,VJM,P/1165 *@CALL TNL@g;
$prog =~ s@14,VJM,P/0021 *@CALL SETNL@g;

# Converting NEW

$prog =~ s@10,VTM,(\d+);14,VJM,P/0206 *;@new(R12=\1);@g;
$prog =~ s@10,VTM,(\d+);14,VJM,P/0025 *;@new(R12=\1);@g;
...
#    print STDERR "At line $line stack is ", join (' // ', @stack), "\n" if @stack;
    if ($line =~ m@(\d+),VTM,([^;]+)@) {
        $regs[$1] = "&$2";
        push @to, "R$1 := &$2"; #  // " . sprintf("L%o", $2);
        ++$from;
        next;
    }
    if ($line =~ m@,ITA,([^;]+)@) {
    ...
    if (@stack >= 2 && $line =~ m@CALL P/IN@) {
        $stack[$#stack-1] = "($stack[$#stack-1] _IN $stack[$#stack])";
        --$#stack;
        ++$from;
        next;
    }
...
#        --$#stack;
    ...
    if (@stack >= 2 && $line =~ m@CALL P/DI@) {
        $stack[$#stack-1] = "($stack[$#stack-1] _DIV $stack[$#stack])";
        --$#stack;
        ++$from;
        next;
    }
    ...
    if (@stack >= 2 && $line =~ m@12,VJM,P/MD@) {
        $stack[$#stack-1] = "($stack[$#stack-1] _MOD $stack[$#stack])";
        --$#stack;
        ++$from;
        next;
    }
    ...
    if ($line =~ m@(\d+),UTM,(\d+)@ && $regs[$1] ne '') {
        push @to, "R$1 := $regs[$1] + $2";
	++$from;
        next;
    }
    ...
    if (@stack >= 2 && $line =~ m@15,C(..),0?$@) {
        ...
        ++$from;
        next;
    } elsif ($line =~/13,UJ,/ && $regs[13] ne '') {
        push @to, "_case $regs[13] _of";
        ++$from;
        next;
    }
...
$prog =~ s@ _IN \(([0-7]+)C\)@" _IN ".convertIntSet($1)@ge;
...
#   var1   := start2   loop3               cond4  end5            exit6 body7
 s@([^;]+) := ([^;]+);L(\d+):,BSS,;_if \(\1 ([<>]) ([^;]+)\) _then goto L(\d+);(.*);\1 := .\1 [-+] .1C?..;,UJ,L\3;L\6@
       "_for $1 := $2 ". ($4 eq '>' ? '_to ' : '_downto ') . "$5 _do _(;$7; (* for $3 *) _);L$6"@ge) { }
...
sub getstring {
    ...
    my $s = "";
    for (; $len > 0; ) {
        if ($prog =~ m@;/$addr:,BSS,;,[GI][OT][SM]T?, *\|([^|]+)\|@) {
            $s .= $1;
            ++$addr; $len -= 6;
        } else { return "string($addr, $len)"; }
    }
...
sub section {
    return $_[0] < $code ? "/$_[0]" : sprintf("L%o", $_[0]);
}
$prog =~ s@R13 := &(\d+);10,VJM,P/0066@"BIND('".getstring($1,6)."')"@ge;
$prog =~ s/R10 := &-(\d+);R13 := &(\d+);writeString/"write('".getstring($2, $1)."')"/ge;

$prog =~ s@R10 := &(\d+);R9 := \&(\d+);P/A7\( \((\d+)C\), \(\3\) \)@"write('".getstring($2,$1)."':".oct($3).")"@ge;
$prog =~ s@R10 := &(\d+);R9 := \&(\d+);P/A7\(([^;]*)\)@"write('".getstring($2,$1)."':($3))"@ge;
...
$prog =~ s@R13 := &(\d+)@"R13 := ".section($1)@ge;

# $prog =~ s/output@ := \((\d+)C\);put.output./"write('".gost(oct($1))."')"/ge;

# Converting remaining stray ATX to assignments
$prog =~ s@TNL; *,ATX,([^;]+)@TNL(\1)@g;
$prog =~ s@,ATX,([^;]+)@\1 := @g;

# Attempting to convert _or
$prog =~ s@if([^;]+) _then goto (L\d+);_if([^;]+) _then goto \2;@if (\1 _or \3) _then goto \2;@g;

#Converting single-statement ifs
$prog =~ s@if([^;]+)goto (L\d+);([^J;]+);\2:@if _not\1 \3;\2:@g;

#Converting some ifs without labels in the cond. statement
$prog =~ s@if([^;]+)goto (L\d+);([^BS]+);\2:@if _not\1 _(;\3;_);\2:@g;

#Converting trivial passing by reference

$prog =~ s@\&\*\(\&([a-z0-9]+)\+0\)@\1@g;

$prog =~ s@(L\d+):,BSS,;_if([^;]*)_then goto (L\d+);(.*;),UJ,\1;\3:@\1:,BSS,;_while _not\2_do _(;\4 (* while \1 *) _);\3:@g;

$prog =~ s@(L\d+):,BSS,;_if([^;]*)_then  _\(;(.*;),UJ,\1;_\);@\1:,BSS,;_while \2_do _(;\3_);@g;

#Fixing calls to pck/unpck
$prog =~ s/R13 := ([^;]*);([^;]*) := pck/pck(\1, \2)/g;
$prog =~ s/R13 := ([^;]*);unpck\(/unpck(\1, /g;

$prog =~ s/R13 := ([^;]*);get/get(\1)/g;

$prog =~ s/@\.f\[0\]/@/g;
$prog =~ s/get\(\&/get(/g;

$prog =~ s/output@ := ([^;]*);put\(output\)/write(\1)/g;

$prog =~ s/writeInt\(([^,;]*), ([^;]*).;/write(\2 : \1);/g;
$prog =~ s/writeCharWide\(([^,;]*), ([^;]*).;/write(chr(\2) : \1);/g;
...
sub mkvars {
    my ($l, $n) = @_;
    return "" if $n == 0;
    if ($n > 100) { return mkvars($l, 100) . "l${l}v101z: _array [101..$n] _of integer;"; }
    my $ret = "_var l$_[0]v1z";
    if ($_[1] <= 100) { map { $ret .= ", l$_[0]v${_}z"; } (2..$_[1]) if $_[1] > 1; }
    return $ret . ":integer;";
}
$prog =~ s/\n (.*): Level (\d) procedure with 0 arguments and (\d+) locals?;/"\n(* Level $2 *) _proced $1;\n".mkvars($2,$3)." _("/ge;
$prog =~ s/\n (.*): Level (\d) procedure with (\d) arguments? and (\d+) locals?;/"\n(* Level $2 *) _proced $1".mkargs($2, $3).";\n".mkvars($2, $4)." _("/ge;
$prog =~ s/\n (.*): Level (\d) function with (\d) arguments? and (\d+) locals?;/"\n(* Level $1 *) _function $1".mkargs($2, $3).":integer;\n".mkvars($2, $4)." _("/ge;

$prog =~ s/EXIT;\n ===/_);\n ===/g;

$prog =~ s/'0+([^']+)'/'\1'/g;

# Fixing vars too far in the global segment
$prog =~ s/\*\(\&(g\d+z)\+0\)/\1/g;
